...
namespace TecnoMundo.Identity.Repository
{
    public class DbRepository : IDbRepository
    {
        ...
        public DbRepository(MySQLContext context, IMapper mapper)
        {
            _context = context;
            _mapper = mapper;
        }
        ...
        {
            return await _context.Users.Where(x => x.Cpf == cpf).AsNoTracking().AnyAsync();
        }

        public async Task<bool> EmailExists(string email)
        {
            return await _context.Users.Where(x => x.UserEmail == email).AsNoTracking().AnyAsync();
        }

        public async Task<bool> TelephoneExists(string phone)
        {
            return await _context
                .Users.Where(x => x.PhoneNumber == phone)
                .AsNoTracking()
                .AnyAsync();
        }
